const IntroductoryText = ({ title = "Título", subtitle, description, bgTitle = "Título", justify = "start", children }) => {
  return (
    <section className="relative">
      {/* Desktop BgTitle */}
      <div className="hidden tablet:block absolute left-0 right-0 -z-10 px-4 desktop:px-8">
        <h1 className={`leading-tight text-${justify} text-[40px] tablet:text-[80px] desktop:text-[140px] bg-clip-text text-transparent bg-gradient-to-t from-lacampana-gray4/20 to-lacampana-gray4`}>{bgTitle}</h1>
      </div>
      {/* Movile BgTitle */}
      <div className="tablet:hidden absolute left-0 right-0 -z-10 px-4 desktop:px-8">
